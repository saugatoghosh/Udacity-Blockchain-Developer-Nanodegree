...
contract('ERC721Mintable', accounts => {
    ...
    describe('match erc721 spec', function () {
        beforeEach(async function () { 
            ...
            let resultToken1 = await this.contract.mint(account_one, 1);
            let resultToken2 = await this.contract.mint(account_one, 2);
            let resultToken3 = await this.contract.mint(account_one, 3);
            let resultToken4 = await this.contract.mint(account_one, 4);
        })
    ...
    describe('have ownership properties', function () {
        ...
        it('should fail when minting when address is not contract owner', async function () { 
            let reverted = false;
            try{
               let resultMint4 = await this.contract.mint(account_one, 4, {from: account_two}); 
            } catch (e) {
                reverted = true;
            }
            assert.equal(reverted, true, "It allows a non contract owner to mint.");
